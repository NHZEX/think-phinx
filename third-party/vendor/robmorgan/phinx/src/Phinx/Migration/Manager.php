...
use Phinx\Util\Util;
use _Z_PhinxVendor\Psr\Container\ContainerInterface;
...
class Manager
{
    ...
    public function migrate(string $environment, ?int $version = null, bool $fake = \false) : void
    {
        ...
        $direction = $version > $current ? \Phinx\Migration\MigrationInterface::UP : \Phinx\Migration\MigrationInterface::DOWN;
        if ($direction === \Phinx\Migration\MigrationInterface::DOWN) {
            // run downs first
            \krsort($migrations);
            foreach ($migrations as $migration) {
                ...
                if (\in_array($migration->getVersion(), $versions)) {
                    $this->executeMigration($environment, $migration, \Phinx\Migration\MigrationInterface::DOWN, $fake);
                }
            }
        }
        \ksort($migrations);
        foreach ($migrations as $migration) {
            ...
            if (!\in_array($migration->getVersion(), $versions)) {
                $this->executeMigration($environment, $migration, \Phinx\Migration\MigrationInterface::UP, $fake);
            }
        }
    }
    /**
     ...
     * @return void
     */
    public function executeMigration(string $name, \Phinx\Migration\MigrationInterface $migration, string $direction = \Phinx\Migration\MigrationInterface::UP, bool $fake = \false) : void
    {
        ...
        $this->printMigrationStatus($migration, $direction === \Phinx\Migration\MigrationInterface::UP ? 'migrating' : 'reverting');
        // Execute the migration and log the time elapsed.
        $start = \microtime(\true);
        $this->getEnvironment($name)->executeMigration($migration, $direction, $fake);
        $end = \microtime(\true);
        $this->printMigrationStatus($migration, $direction === \Phinx\Migration\MigrationInterface::UP ? 'migrated' : 'reverted', \sprintf('%.4fs', $end - $start));
    }
    /**
    ...
    public function executeSeed(string $name, SeedInterface $seed) : void
    {
        ...
        $end = \microtime(\true);
        $this->printSeedStatus($seed, 'seeded', \sprintf('%.4fs', $end - $start));
    }
    /**
     ...
     * @return void
     */
    protected function printMigrationStatus(\Phinx\Migration\MigrationInterface $migration, string $status, ?string $duration = null) : void
    {
        $this->printStatusOutput($migration->getVersion() . ' ' . $migration->getName(), $status, $duration);
    }
    /**
     * Print Seed Status
     *
    ...
    {
        $this->printStatusOutput($seed->getName(), $status, $duration);
    }
    /**
    ...
    {
        $this->getOutput()->writeln(' ==' . ' <info>' . $name . ':</info>' . ' <comment>' . $status . ' ' . $duration . '</comment>', $this->verbosityLevel);
    }
    /**
     ...
     * @return void
     */
    public function rollback(string $environment, $target = null, bool $force = \false, bool $targetMustMatchVersion = \true, bool $fake = \false) : void
    {

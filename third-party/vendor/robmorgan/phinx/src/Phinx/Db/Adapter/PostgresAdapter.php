...
class PostgresAdapter extends \Phinx\Db\Adapter\PdoAdapter
{
    ...
    public function connect() : void
    {
        if ($this->connection === null) {
            ...
            try {
                ...
                throw new InvalidArgumentException(\sprintf('Schema does not exists: %s', $options['schema']), 0, $exception);
            }
    ...
    public function createTable(Table $table, array $columns = [], array $indexes = []) : void
    {
        ...
        if (isset($options['id']) && \is_string($options['id'])) {
            ...
            $column = new Column();
            $column->setName($options['id'])->setType('integer')->setOptions(['identity' => \true]);
    ...
    public function getColumns(string $tableName) : array
    {
        ...
        $sql = \sprintf('SELECT column_name, data_type, udt_name, is_identity, is_nullable,
             ...
             ORDER BY ordinal_position', $this->useIdentity ? ', identity_generation' : '', $this->getConnection()->quote($parts['schema']), $this->getConnection()->quote($parts['table']));
    ...
    {
        $instructions = new AlterInstructions();
        $instructions->addAlter(\sprintf('ADD %s %s %s', $this->quoteColumnName($column->getName()), $this->getColumnSqlDefinition($column), $column->isIdentity() && $column->getGenerated() !== null && $this->useIdentity ? \sprintf('GENERATED %s AS IDENTITY', $column->getGenerated()) : ''));
        if ($column->getComment()) {
    ...
    public function getSqlType($type, ?int $limit = null) : array
    {
        switch ($type) {
            ...
            default:
                ...
                // Return array type
                throw new \Phinx\Db\Adapter\UnsupportedColumnTypeException('Column type `' . $type . '` is not supported by Postgresql.');
        }
    }
    /**
    ...
    public function getPhinxType(string $sqlType) : string
    {
        switch ($sqlType) {
            ...
            default:
                throw new \Phinx\Db\Adapter\UnsupportedColumnTypeException('Column type `' . $sqlType . '` is not supported by Postgresql.');
        }
    }
    /**
     * @inheritDoc

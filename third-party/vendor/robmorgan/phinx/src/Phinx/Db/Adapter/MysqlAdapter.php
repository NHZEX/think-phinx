 ...
 */
class MysqlAdapter extends \Phinx\Db\Adapter\PdoAdapter
{
    /**
     * @var string[]
     */
    ...
    public function createTable(Table $table, array $columns = [], array $indexes = []) : void
    {
        ...
        $defaultOptions = ['engine' => 'InnoDB', 'collation' => 'utf8mb4_unicode_ci'];
        ...
        if (isset($options['id']) && \is_string($options['id'])) {
            ...
            $column = new Column();
            $column->setName($options['id'])->setType('integer')->setOptions(['signed' => $options['signed'] ?? !FeatureFlags::$unsignedPrimaryKeys, 'identity' => \true]);
    ...
    public function getSqlType($type, ?int $limit = null) : array
    {
        switch ($type) {
            ...
            default:
                throw new \Phinx\Db\Adapter\UnsupportedColumnTypeException('Column type "' . $type . '" is not supported by MySQL.');
        }
    }
    /**
    ...
    public function getPhinxType($sqlTypeDef)
    {
        ...
        if (!\preg_match('/^([\\w]+)(\\(([\\d]+)*(,([\\d]+))*\\))*(.+)*$/', $sqlTypeDef, $matches)) {
            throw new \Phinx\Db\Adapter\UnsupportedColumnTypeException('Column type "' . $sqlTypeDef . '" is not supported by MySQL.');
        }
        $limit = null;
        $scale = null;
        $type = $matches[1];

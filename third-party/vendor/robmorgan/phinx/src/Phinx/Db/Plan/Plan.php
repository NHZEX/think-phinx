...
class Plan
{
     ...
     */
    public function __construct(\Phinx\Db\Plan\Intent $intent)
    {
     ...
     */
    protected function remapContraintAndIndexConflicts(\Phinx\Db\Plan\AlterTable $alter) : \Phinx\Db\Plan\AlterTable
    {
        $newAlter = new \Phinx\Db\Plan\AlterTable($alter->getTable());
    ...
    protected function forgetDropIndex(Table $table, array $columns, array $actions) : array
    {
        ...
        $indexes = \array_map(function ($alter) use($table, $columns, $dropIndexActions) {
            ...
            $newAlter = new \Phinx\Db\Plan\AlterTable($table);
            foreach ($alter->getActions() as $action) {
    ...
    protected function gatherCreates(array $actions) : void
    {
        foreach ($actions as $action) {
            if ($action instanceof CreateTable) {
                $this->tableCreates[$action->getTable()->getName()] = new \Phinx\Db\Plan\NewTable($action->getTable());
            }
        }
        foreach ($actions as $action) {
        ...
        foreach ($actions as $action) {
            ...
            if ($action instanceof RemoveColumn) {
                if (!isset($this->columnRemoves[$name])) {
                    $this->columnRemoves[$name] = new \Phinx\Db\Plan\AlterTable($table);
                }
                $this->columnRemoves[$name]->addAction($action);
            } else {
                if (!isset($this->tableUpdates[$name])) {
                    $this->tableUpdates[$name] = new \Phinx\Db\Plan\AlterTable($table);
                }
    ...
    protected function gatherTableMoves(array $actions) : void
    {
        foreach ($actions as $action) {
            ...
            if (!isset($this->tableMoves[$name])) {
                $this->tableMoves[$name] = new \Phinx\Db\Plan\AlterTable($table);
            }
    ...
    protected function gatherIndexes(array $actions) : void
    {
        foreach ($actions as $action) {
            ...
            if (!isset($this->indexes[$name])) {
                $this->indexes[$name] = new \Phinx\Db\Plan\AlterTable($table);
            }
    ...
    protected function gatherConstraints(array $actions) : void
    {
        foreach ($actions as $action) {
            ...
            if (!isset($this->constraints[$name])) {
                $this->constraints[$name] = new \Phinx\Db\Plan\AlterTable($table);
            }
